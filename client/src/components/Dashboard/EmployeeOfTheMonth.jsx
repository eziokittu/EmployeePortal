...
const EmployeeOfTheMonth = () => {
  ...
  return (
    <div className="p-4 sm:ml-64">
      <div className="p-4 border-2 border-gray-300 bg-slate-100 rounded-lg text-center">

        {/* Heading */}
        <div className='font-bold text-3xl mt-4 mb-2'>Employee of the month</div>
        <div className='font-bold text-xl mb-8'>{`${currentMonth}`}</div>
        ...
        <div>
          ...
          {!loadedEmpMonth && (
            <div className='bg-white px-2 py-4 border border-gray-600 my-4 rounded-lg'>{`The list for Employee of the month for ${currentMonthFull} has not been added by ADMIN!`}</div>
          )}
        </div>

        {/* ADD emp month button for ADMIN */}
        {auth.isAdmin && (
          <EmpMonthAddEmp monthYear={formattedDate}/>
        )}

        {/* Buttons to view Employee of the month for other months */}
        <div className='mt-8'>
          <EmpMonthListByYear reloadNeeded={false} startYear={'19/04/2023'} endYear={new Date().getDate()}/>
        </div>
      </div>
    </div>
  )
